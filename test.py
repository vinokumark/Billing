...
curs = con.execute("select item_list from list")
cur = curs.fetchall()

# con.row_factory = sqlite3.Row
# t = con.cursor()
# ci = t.execute("select item_name from list")
# si = ci.fetchall()
# outp = si.()
...
def login(btn):
    if btn == "Clear":
        ...
        app.entry("password", "")
          ...
          if username and password == "vinoth":
            ...
            app.errorBox("Error", "Your credentials are invalid !")
...
def toolbar(btn):
 ...
 #   elif btn == "fileimport":app.openBox(title=None,dirName=None,fileTypes=[('csv','*.csv')],asFile=False,parent=None)
...
def listofma():
     app.label("testing",cur)

def add_item():
      app.addAutoEntry("customer1",outp,1,0)
#     app.setTabbedFrameSelectedTab

def fileimport():
    files = app.openBox(title=None, fileTypes=[('csv','*.csv')], asFile=None,parent=None)
    with open(files,'rt') as op:
         ts = csv.reader(op)
         for ins in ts:
#             print(ins)
              new = sqlite3.connect('billing.db')
              im= new.cursor()
              im.execute('INSERT INTO list(item_list,quantity,price) VALUES(?,?,?);' , ins)
              new.commit()
              new.close()
...
with gui("ShowCase") as app:
    app.showSplash("Maayukam")

    # add a simple toolbar
    app.addToolbar(["EXIT", "LOGOUT", "FILL", "ACCESS", "PIE-CHART", "CALENDAR", "ADDRESS-BOOK", "MAP", "FULL-SCREEN", "fileimport"], toolbar, findIcon=True)
    ...
    app.addMenuList("Tabs", ["Login", "Lists", "Properties", "Meters", "Drag'nDrop", "Calculator", "Panes", "Labels", "LIST Material"], changeTab)
 ...
 #   app.addMenuCheckBox("Boxes", "Box 6", toolbar, fileimport)
    ...
    app.addMenuList("List Items", ["Item 1", "Item 2", "-", "Item 3", "Item 4", "Item 5", "Item 6", "Item 7" ,'fileimport'], toolbar)
    app.addMenuList("List Items", ["-", "aaa", "-", "bbb", "-", "ccc", "-", "ddd", "-"], toolbar)
    app.addMenuList("IMPORT",["imports"],[fileimport])
    ...
    with app.tabbedFrame("Tabs"):
        ...
        with app.tab("List Material"):
            #  app.label("test")
             app.button("mlist",listofma)
             app.addAutoEntry("title",outp)
        
        with app.tab("Billing"):
            app.addLabel("customer_no","Customer No",0,0)
            app.addAutoEntry("customer",outp,0,1)
            app.addButton("add item",add_item)
            app.addAutoEntry("customer1",outp,0,1)
             
#             app.addListBox("itemlist",[te])
